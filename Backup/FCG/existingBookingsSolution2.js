async function handler(C) {
    ...
    function checkBookingConflict(newBooking, existingAppointments) {
        ...
        for (const booking of existingAppointments) {
            ...
            // Allow if new start time is the same as the existing appointment's end time
            if (
                !newStartTime.isSame(existingEndTime) && // Allow if new start time is exactly the same as existing end time
                newStartTime.isBetween(existingStartTime, existingEndTime, null, "[)")
            ) {
                conflict.startTimeConflict = true;
            }

            // Allow if new end time is the same as the existing appointment's start time
            if (
                !newEndTime.isSame(existingStartTime) && // Allow if new end time is exactly the same as existing start time
